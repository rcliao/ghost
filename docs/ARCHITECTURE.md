...
| `internal/mcpserver` | MCP server over stdio (11 tools: put, patch, get, search, context, expand, consolidate, curate, reflect, edge, edge_candidates) | ~556 LOC |
...
- `ReviewRuleEvent` records `agree` or `disagree`. Agreeing with a proposal is what writes the edge; disagreeing removes an edge whether a rule asserted it or a reviewer accepted it.
- Retrieval consults the trace in exactly one place: edge expansion reads the set of agreed edges once per `Context` call, and an agreed edge may claim reserved budget even when its relation is accompany-class. Measured on a production snapshot at production settings, sixteen reviewed `caused_by` edges pulled their cause into context 6/16 times against 0/16 unreviewed, with no regressions; the in-repo eval is `TestEvalReviewedEdgeSurvivesTightBudget`. Unreviewed edges get no reservation, so the reserve class stays scarce.
...
- `ghost_patch` — Apply a unified diff to a memory's content; refuses on context mismatch or a version conflict instead of corrupting (see `ghost patch` below)
- `ghost_get` — Retrieve a specific memory by namespace and key
- `ghost_search` — Full-text search with ranking
- `ghost_context` — Budget-aware context assembly with edge expansion (includes `compaction_suggested` signal; supports `min_score` / `min_spread` noise filters, and `for_user` / `for_scope` boosts for the current interlocutor / encoding scope)
...
**Why the tiers read different inputs.** The mechanical tier reads a prompt buffer that `hooks/ghost-user-prompt.sh` appends `.prompt` to on every UserPromptSubmit — not the session transcript. A Claude Code transcript labels far more than user speech as `type: "user"`: harness control messages (`<task-notification>`, slash-command caveat/stdout wrappers), hook-injected context, skill bodies, and tool results all arrive that way. Salience scoring cannot tell those from prose, so transcript-fed mechanical capture stored schema fragments and skill documentation as memories, which retrieval then served back into every session. Successive pattern-strip guards did not hold — junk regenerated under new keys. Reading the prompt buffer removes that class of input by construction. The LLM tier still reads the full transcript, where a model can distinguish synthesis from noise.
