...
| [Hindsight Comparison](hindsight-comparison.md) | Research | How vectorize-io/hindsight works beside ghost: Postgres server, LLM-mandatory writes, no lifecycle, no pair-rules analogue; its retrieval layer (rank-space boosting, level knobs, bounded boosts, interleave fusion) described, not scheduled |
